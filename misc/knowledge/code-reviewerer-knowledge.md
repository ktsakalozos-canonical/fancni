...
- **IPv6 Inputs:** The `ComputeSubnet`, `ComputeGateway`, and `validateOverlay` functions currently reject IPv6 overlays using `To4() == nil`. Improve error handling to provide explicit feedback for IPv6 inputs.
- **Overlay CIDR Validation:** The overlay CIDR is validated only for the first octet. Ensure that the overlay CIDR is validated to confirm it is indeed /8.
- **Bridge Name Collisions:** The `ComputeBridgeName` function generates bridge names based solely on the first octet of the overlay, which can lead to collisions. Implement a more robust naming strategy to avoid collisions.
